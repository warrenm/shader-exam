...
class Renderer: NSObject, MTKViewDelegate {
    ...
    let SHADER_RELOAD_FRAME = 10

    let view: MTKView
    let device: MTLDevice
    let commandQueue: MTLCommandQueue
    var mainRenderPipeline: MTLRenderPipelineState?
    var postRenderPipeline: MTLRenderPipelineState?
    ...
    init(view: MTKView, device: MTLDevice) {
        ...
        guard let vertexDescriptor_ = MTKMetalVertexDescriptorFromModelIO(mdlVertexDescriptor) else {
        ...
        quadVertexDescriptor = MTLVertexDescriptor()
    ...
    func draw(in view: MTKView) {
        frameCount += 1
        if frameCount % SHADER_RELOAD_FRAME == 0 {
            _ = loadShaders()
        }
    ...
    func loadShaders() -> Bool {
        let shadersPath = projectDir + "/ShaderExam/Shaders.metal"
        let quadShadersPath = projectDir + "/ShaderExam/QuadShaders.metal"
        ...
        if shaderString == shaderStringNew {
            return true
        } else {
            shaderString = shaderStringNew
        }

        guard let library = try? device.makeLibrary(source: shaderString, options: nil) else {
            print("Could not compile Metal library from shader file")
            return false
        }
